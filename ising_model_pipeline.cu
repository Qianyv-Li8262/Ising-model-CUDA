...
__shared__ uint32_t tile[34][34];


if (in_bounds){
int self = y_lineid * x_words_num + x_wordid;
int left = x_wordid == 0 ? (y_lineid+1) * x_words_num - 1 : x_words_num*y_lineid+x_wordid-1;
int right = x_wordid == x_words_num-1 ? (y_lineid) * x_words_num : x_words_num*y_lineid+x_wordid+1;
int up = y_lineid == 0 ? x_words_num*(y_lines-1)+x_wordid : x_words_num*(y_lineid-1)+x_wordid;
int down = y_lineid == y_lines - 1 ? x_wordid : x_words_num*(y_lineid+1)+x_wordid;
__pipeline_memcpy_async(&tile[threadIdx.y+1][threadIdx.x+1],&spin_field[self],sizeof(uint32_t));
if(threadIdx.x==0) __pipeline_memcpy_async(&tile[threadIdx.y+1][0],&spin_field[left],sizeof(uint32_t));
if(threadIdx.y==0) __pipeline_memcpy_async(&tile[0][threadIdx.x+1],&spin_field[up],sizeof(uint32_t));
if(threadIdx.x==blockDim.x-1) __pipeline_memcpy_async(&tile[threadIdx.y+1][blockDim.x+1],&spin_field[right],sizeof(uint32_t));
if(threadIdx.y==blockDim.y-1) __pipeline_memcpy_async(&tile[blockDim.y+1][threadIdx.x+1],&spin_field[down],sizeof(uint32_t));
__pipeline_commit();
int s = y_lineid * x_words_num + x_wordid+1;
pcg32_state pcg_rng;
...
uint32_t self_word      = tile[threadIdx.y + 1][threadIdx.x + 1];
uint32_t up_neighbor    = tile[threadIdx.y][threadIdx.x + 1];
uint32_t down_neighbor  = tile[threadIdx.y + 2][threadIdx.x + 1];
uint32_t left_neighbor  = tile[threadIdx.y + 1][threadIdx.x];
uint32_t right_neighbor = tile[threadIdx.y + 1][threadIdx.x + 2];
